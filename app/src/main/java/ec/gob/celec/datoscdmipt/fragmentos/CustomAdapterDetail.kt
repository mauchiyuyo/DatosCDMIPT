...
class CustomAdapterDetail(context: Context, items: List<JSONObject>) : ArrayAdapter<JSONObject>(context, 0, items){
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        txtIPTid?.text = currentItem?.getString("STRING_MRID")

        val txtNombreIPT = itemView?.findViewById<TextView>(R.id.txtNombreIPT)
        txtNombreIPT?.text = currentItem?.getString("VALUEORIG")
